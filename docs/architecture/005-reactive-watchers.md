...
1. **Monitoramento Recursivo em FS**: O `Watcher` base (`fsnotify`) engatilha callbacks monitorando toda a árvore sob gestão.
2. **Git Awareness (Evitando Event Storms)**: Se o diretório base for um repositório Git, o watcher monitora agressivamente as chamadas ao `index.lock`. Enquanto o Git executa transações densas em massa (ex: um `git checkout` alterando 1.000 arquivos subitamente), o Watcher não satura a aplicação cliente com eventos, silenciando as notificações (`Pause`).
...
* **Pró (Proteção Termodinâmica):** Os "debouncers" de 50ms e buffers protegem a performance da RAM da aplicação de picos excessivos do sistema operacional, mantendo a responsividade do host.
